...
def input_id_student(root, button_text, select_student, purpose):
    ...
    window_width = 800
    window_height = 600
    screen_width = input_id_window.winfo_screenwidth()
    screen_height = input_id_window.winfo_screenheight()
    x = (screen_width // 2) - (window_width // 2)
    y = (screen_height // 2) - (window_height // 2)
    
    input_id_window.geometry(f"{window_width}x{window_height}+{x}+{y}")
    ...
    # Create the entry field for displaying the school ID
    ...
    # Create buttons and bind them to append the respective values to the entry
    ...
    delete_button = ctk.CTkButton(frame, text='Del', fg_color="#d68b26", hover_color="#a45e14", text_color='#fff', border_width=1, border_color='#000', height=60, command=delete_last_char)
    ...
    clear_button = ctk.CTkButton(frame, text='Clear', fg_color="#d68b26", hover_color="#a45e14", text_color='#fff', border_width=1, border_color='#000', height=60, command=clear_entry)
    clear_button.grid(row=5, column=0, columnspan=3, sticky='wnes', pady=5, padx=5)

    # Label text below
    heading_label = ctk.CTkLabel(main_frame, text="Please proceed to create a ticket after entering your", text_color="#000000", anchor="center")
    heading_label.pack(padx=(0,20))
    heading_label = ctk.CTkLabel(main_frame, text="school ID number.", text_color="#000000", anchor="center")
    heading_label.pack(padx=(0,20))

    # Create a button frame for Cancel and Create Ticket buttons
...
def open_ticket_window(student_name, ticket_number, cname):
    import tkinter as tk
    ...
    queue_label.pack(pady=(20,10), anchor="e")
        ...
        # Instructional Text
    instruction_label = tk.Label(root, text=f" {student_name} Click Print Ticket to generate your queue ticket.\n"
